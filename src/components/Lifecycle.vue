...
const lifecycleList :Lifecycle[] = [
  ...
  },
  {
    ...
    desc: '<strong>Vue 인스턴스가 생성된 후, 컴포넌트가 DOM에 마운트 되기 전에 실행</strong> <br><br>- 렌더링 직전에 DOM을 변경하고자 할 떄 사용 ',
    warn: '- 컴포넌트가 DOM에 마운트되기 전, 초기화 작업 수행시 사용 <br>- 외부 API와 통신하여 데이터를 가져올 떄 사용 <br>- Vuex 스토어 초기화, 이벤트 리스너 등록, 다른 컴포넌트와의 상호작용 등의 작업 수행 가능 <br>- DOM 요소를 직접 조작하는 것은 onMounted에서 수행하기(여기서 조작하면 레더링 지연 및 예기치 못한 상황 발생)',
    ex:' <br>ex) Todo List를 불러오는 API호출로 미리 데이터 셋팅하면 더 빠르게 볼 수 있게 됨 <br><br>'
  },
  {
    optApi: 'mounted',
    compApi: 'onMounted',
  ...
  },
  {
    ...
    desc: '<strong>컴포넌트 데이터가 변함으로 인해 DOM이 변화되기 전에 호출(데이터는 변했지만 아직 DOM변화 전)</strong> <br><br>- 컴포넌트가 재랜더링 되기 전에 실행되는 유일한 훅 <br>- 컴포넌트가 변경될 때마다 호출됨<br>- 여기서 데이터 값을 변경시키면 그 값이 렌더링에 반영되고, 다시 이 훅이 실행되진 X(DOM 변경에 영향을 주는 데이터가 변하지 않으면 호출되지 X )',
    warn: '- 변경사항을 로깅하거나 비동기적으로 데이터를 가져오는 작업 수행',
    ex:'<button @click="change">바꾸기</button>'
  },
  {
      optApi: 'updated',
      compApi: 'onUpdated',
      ...
      warn: '- DOM 변경에 영향을 주는 데이터가 변하지 않으면 호출되지 X => 만약, 여기서 값을 변경한다면 다시 beforeUpdate 훅부터 호출 -> 무한루프 <br>- 컴포넌트 상태에 따라 화면에 표시되는 내용이 변경되는 경우' ,
      ex:''
    },
  {
...
let value = ref('onBeforeUpdate');
const change = () => {
  value.value = 'called onBeforeUpdate'
  console.log(value)
}
