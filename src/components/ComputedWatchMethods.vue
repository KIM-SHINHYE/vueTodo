    ...
    <div v-if="props.mydata == 'watch'">
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    <td>반응형 의존성 추적 방식</td>
                    <td>특정 데이터나 속성 감시하고 추적. 그 값이 변경될 때마다 콜백함수 실행.<br>=> 즉, 콜백 내에서 조회하는 항목은 추적하지 않지만 콜백이 실행되어야 하는 시기를 보다 명확하게 제어 가능</td>
                    <td>함수를 인자로 받아 해당 함수 안에서 사용된 모든 반응성 데이터를 자동으로 추적. 추적된 데이터가 변경될 때마다 함수가 다시 실행됨. <br>=> 즉, 동기적 실행 중에 조회되는 모든 반응형 속성을 자동으로 추적하지만 콜백이 실행되어야 하는 시기가 덜 명시적</td>
                </tr>
            </tbody>
        </table>
    </div>
